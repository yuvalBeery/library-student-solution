...
const updateBookV2 = async (id: number, genreId: number): Promise<void> => {
 try {
    ...
    error.driverError.code === PostgresErrors.FOREIGN_KEY_VIOLATION
        ...
        throw new UnprocessableEntityError(`The genre with id ${genreId} was not found`);
    } else {
...
const getMostBorrowsBooksV2 = async (): Promise<Book[]> => {
...
// Re-export functions that are the same in V1 and V2
export { 
  getBookById,
  updateBookV2,
  getMostBorrowsBooksV2
};